...
import static org.deeplearning4j.examples.cifar.CifarDataLoader.IMAGE_SIZE;
import static org.deeplearning4j.examples.cifar.CifarDataLoader.loadCifar10Data;
...
public class NearestNeighbor {
    ...
    private static void processCifar10Images() throws IOException {
        ...
        final byte[] testImageData = loadCifar10Data(false);
        ...
        for (int epochIndex = 0; epochIndex < numberOfEpochs; epochIndex++) {
            ...
            for (int i = 0; i < numberOfImagesToTest; i++) {
                int imageIndex = random.nextInt(10000) * (IMAGE_SIZE + 1);
                final byte[] imageByteArray = Arrays.copyOfRange(testImageData, imageIndex + 1, imageIndex + (IMAGE_SIZE + 1));
                ...
                l1MatchCount += trainingMap.entrySet().stream()
                            .min((o1, o2) -> {
                                ...
                                return (int) (o1Difference - o2Difference);
                            })
    ...
    private static Map<INDArray, Byte> readTrainingData() throws IOException {
        ...
        byte[] trainingImageData = loadCifar10Data(true);;
        for (int imageIndex = 0; imageIndex < trainingImageData.length; imageIndex += (IMAGE_SIZE + 1)) {
            final byte[] imageByteArray = Arrays.copyOfRange(trainingImageData, imageIndex + 1, imageIndex + (IMAGE_SIZE + 1));
            final double[] imageDoubles = IntStream.range(0, imageByteArray.length).mapToDouble(idx -> imageByteArray[idx]).toArray();

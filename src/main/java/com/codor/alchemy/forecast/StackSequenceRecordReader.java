...
public class StackSequenceRecordReader extends FileRecordReader {
	...
	/*public StackSequenceRecordReader(FileSystem fs) {
 	...
 	public StackSequenceRecordReader(FileSystem fs, int startSeq, int endSeq) {
		if (startSeq > endSeq) throw new RuntimeException("Start is greater than End.");
		this.fs = fs;
		for (int i = startSeq; i <= endSeq; i++) {
                        timeSteps.add(String.valueOf(i));
                }	
	...
	private MultiDataSet zeroMDS(int numFeatures, int numLabels) {
		return new org.nd4j.linalg.dataset.MultiDataSet(
                       new INDArray[] { Nd4j.create(new double[numFeatures]) },
	...
	public MultiDataSet toMultiDataSet(int numFeatures, int numLabels) {
		...
		int currHotIndex = 0, predHotIndex = 0;
		for (int i = 0; i < records.size(); i++) {
			...
			for (int j = 0; j < timeSteps.size(); j++) {
				if (featureMasks.get(i).get(j) != 1.0) {
					...
					list.add(new org.nd4j.linalg.dataset.MultiDataSet(
						 new INDArray[] { feature }, new INDArray[] { label },
...
/*	private Pair<INDArray[], INDArray[]> toPair(List<Double> fMask, List<Double> lMask, int size) {
		...
		for (int j = 0; j < fMaskSplits.size(); j++) {
			...
			for (int k = 0; k < fMaskSplits.get(j).size(); k++) {
                        	fMaskArray[k] = fMaskSplits.get(j).get(k);
                		lMaskArray[k] = lMaskSplits.get(j).get(k);
                	}

			array1[j] = Nd4j.create(fMaskArray);
			array2[j] = Nd4j.create(lMaskArray);
		}
	...
	private Pair<INDArray[], INDArray[]> toPair(Collection<Writable> record, int numFeatures, int numLabels) {
		...
		//org.junit.Assert.assertTrue("Count: " + count + " - " + count1 + " - " + count2,
